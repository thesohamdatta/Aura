...
base_url = os.getenv("AURA_API_BASE_URL", "https://api.aura.me/v1/mcp/")
if not base_url or base_url == "":
    raise Exception("Base URL not found")


class AuraTools(str, Enum):
...
class GetMemories(BaseModel):
    api_key: Optional[str] = Field(
        description="The user's MCP API key. If not provided, it will be read from the AURA_API_KEY environment variable. For more details, see https://docs.aura.me/doc/developer/MCP",
        default=None,
    )
...
async def serve(uid: str | None) -> None:
    ...
    server = Server("mcp-aura")
    logger.info("mcp-aura server started")

    @server.list_tools()
    async def list_tools() -> list[Tool]:
        return [
            Tool(
                name=AuraTools.GET_MEMORIES,
            ...
            Tool(
                name=AuraTools.GET_CONVERSATIONS,
            ...
            Tool(
                name=AuraTools.GET_CONVERSATION_BY_ID,
    ...
    async def call_tool(name: str, arguments: dict) -> list[TextContent]:
        ...
        api_key = arguments.get("api_key") or os.getenv("AURA_API_KEY")
        if not api_key:
            raise ValueError("API key not provided and AURA_API_KEY environment variable not set.")

        if name == AuraTools.GET_MEMORIES:
        ...
        elif name == AuraTools.CREATE_MEMORY:
        ...
        elif name == AuraTools.DELETE_MEMORY:
            result = delete_memory(api_key, memory_id=arguments["memory_id"])
            return [TextContent(type="text", text=json.dumps(result, indent=2))]

        elif name == AuraTools.EDIT_MEMORY:
            result = edit_memory(
                api_key,
        ...
        elif name == AuraTools.GET_CONVERSATIONS:
        ...
        elif name == AuraTools.GET_CONVERSATION_BY_ID:
