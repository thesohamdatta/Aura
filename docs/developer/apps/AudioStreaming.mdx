...
description: "Stream raw audio bytes from your Aura device to any backend for custom speech processing, VAD, or audio analysis."
---

## Overview

Aura allows you to stream audio bytes from your DevKit directly to your backend or any external service. This enables custom audio processing like:

<CardGroup cols={2}>
  <Card title="Custom Speech Recognition" icon="microphone">
    Use your own ASR models instead of Aura's default transcription
  </Card>
...
flowchart LR
    ...
    A -->|Every X seconds| B[Aura Backend]
...
<Note>
  The sample rate is passed as a query parameter so your endpoint can handle
...
<Steps>
  ...
  <Step title="Configure in Aura App" icon="mobile">
    1. Open the Aura App
  ...
  <Step title="Test Your Integration" icon="flask">
    Start speaking while wearing your Aura device. Audio bytes should arrive at your webhook at the configured interval.
...
<Steps>
  ...
  <Step title="Fork the Example Repository" icon="code-fork">
    Fork [github.com/mdmohsin7/aura-audio-streaming](https://github.com/mdmohsin7/aura-audio-streaming)
  </Step>
  ...
  <Step title="Configure Aura App" icon="mobile">
...
<CardGroup cols={2}>
  ...
  <Card title="Handle Missing Data" icon="triangle-exclamation">
    Network issues may cause gaps. Design your processing to handle incomplete
    audio.
  </Card>
  <Card title="Buffer Appropriately" icon="layer-group">
    Choose chunk interval based on your use case. Larger chunks = fewer requests
    but higher latency.
  </Card>
  <Card title="Monitor Usage" icon="chart-line">
    Audio streaming generates significant data. Monitor storage and bandwidth
    costs.
  </Card>
</CardGroup>

<Warning>
  Audio data is sensitive. Ensure your endpoint is secured with HTTPS and
...
<CardGroup cols={2}>
  ...
  <Card
    title="Saving Audio"
    icon="floppy-disk"
    href="/doc/developer/savingaudio"
  >
  ...
  >
    How Aura's built-in transcription works
  </Card>
  <Card
  ...
  >
    Overview of all Aura app types
  </Card>
</CardGroup>
