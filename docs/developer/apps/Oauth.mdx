...
description: "Integrate your application with Aura using OAuth 2.0 to securely access user data with explicit consent."
---

## Overview

Aura uses an OAuth 2.0-like flow to allow third-party applications to access user data with explicit consent. This ensures users maintain control over their data and what applications can access.

```mermaid
sequenceDiagram
    participant App as Your App
    participant User as User Browser
    participant Aura as Aura Auth Server
    participant Firebase as Firebase Auth

    App->>User: Redirect to /v1/oauth/authorize
    User->>Aura: Visit authorization page
    Aura->>Firebase: User logs in (Google/Apple)
    Firebase-->>Aura: Firebase ID token
    Aura->>Aura: Exchange token for uid
    Aura->>User: Redirect to App Home URL
...
<Steps>
  <Step title="Register Your App" icon="id-card">
    Your app must be registered with Aura and have an **App ID**
  </Step>
  <Step title="Configure App Home URL" icon="link">
    Set your **App Home URL** in your app's settings - this is where users are
...
<Steps>
  <Step title="Redirect User to Aura" icon="arrow-right">
    Your app redirects the user to Aura's authorization endpoint:

    ```
    https://api.aura.me/v1/oauth/authorize?app_id=YOUR_APP_ID&state=YOUR_STATE
    ```
  ...
  <Step title="User Authenticates & Consents" icon="user-check">
    On the Aura authorization page, the user:
    ...
    **Example permissions shown to users:**
    - "Engage in chat conversations with Aura"
  ...
  <Step title="Token Exchange" icon="arrows-rotate">
    After user approval, Aura's authorization page handles the token exchange internally:

    ```http
    POST https://api.aura.me/v1/oauth/token
    ...
    **Response:**
    ...
    <Note>
    This step is handled automatically by Aura's authorization page - you don't need to implement this yourself.
    </Note>
...
<Tabs>
  <Tab title="Python (FastAPI)" icon="python">
    ...
    async def start_oauth():
        ...
        return RedirectResponse(
            f"https://api.aura.me/v1/oauth/authorize"
  ...
  <Tab title="Node.js (Express)" icon="node-js">
    ...
    app.get('/start-oauth', (req, res) => {
      ...
      // Redirect to Aura
      res.redirect(
        `https://api.aura.me/v1/oauth/authorize` +
...
When a user completes the OAuth flow, Aura automatically attempts to enable your app for them.

<AccordionGroup>
  <Accordion title="Enablement Checks" icon="list-check">
    Aura performs several checks before enabling:
  ...
  <Accordion title="Setup Completion Check" icon="gear">
    ...
    **Expected Response:**
    ...
    Return `false` if the user hasn't completed setup yet. Aura will show an appropriate message.
...
Configure these settings in the Aura developer portal or during app submission:
...
<CardGroup cols={2}>
  <Card title="Always Use State" icon="shield">
    Generate a cryptographically random `state` parameter and validate it on
...
<Warning>
  Never expose your App ID in client-side code where it could be extracted.
  While the App ID itself isn't secret, it should be treated as sensitive for
  your integration's integrity.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Setup completion check failing" icon="circle-question">
    **Possible causes:**
    ...
    - Endpoint not accessible from Aura servers
