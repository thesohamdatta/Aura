...
description: "Build webhook-based apps that connect Aura to external services. Process memories, real-time transcripts, or raw audio."
---

## What Are Integration Apps?

Integration apps allow Aura to interact with external services by sending data to your webhook endpoints. Unlike prompt-based apps, these require you to host a server.
...
These apps are activated when Aura creates a new memory, allowing you to process or store the data externally.

<AccordionGroup>
  <Accordion title="How It Works" icon="gear">
    1. User completes a conversation
    2. Aura processes and creates a memory
...
<Tip>
  Check out the [Notion CRM
  Example](https://github.com/BasedHardware/Aura/blob/main/plugins/example/main.py)
...
<AccordionGroup>
  <Accordion title="How It Works" icon="gear">
    ...
    2. Aura transcribes in real-time
...
<Tip>
  See the [Real-time News Checker
  Example](https://github.com/BasedHardware/aura/blob/main/plugins/example/advanced/realtime.py)
  for a complete implementation.
</Tip>
...
Stream raw audio bytes from Aura directly to your endpoint for custom audio processing.

<AccordionGroup>
  <Accordion title="How It Works" icon="gear">
    1. User speaks into Aura device
...
<Note>
  To produce a playable WAV file, prepend a WAV header and concatenate the
  received chunks.
</Note>

### Configuring Delivery Frequency

You can control how often audio is sent via the Aura app Developer Settings:
...
<Info>
  For a complete implementation, see the [Audio Streaming
  Guide](/doc/developer/apps/AudioStreaming).
</Info>
...
<Steps>
  ...
  <Step title="Submit Your App" icon="rocket">
    Publish through the Aura mobile app
  </Step>
</Steps>
...
<Steps>
  <Step title="Enable Developer Mode" icon="code">
    Open Aura app → Settings → Enable Developer Mode → Developer Settings
  </Step>
  ...
  <Step title="Test Memory Triggers" icon="bell">
    Go to any memory → Tap 3-dot menu → Developer Tools → Trigger webhook with
    existing data
  </Step>
...
<Tip>
  Use [webhook.site](https://webhook.site) to see exactly what data Aura sends
...
<Info>
  When users open your setup links, Aura automatically appends a `uid` query
  parameter. Use this to associate credentials with specific users.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="/doc/developer/apps/PromptBased"
  >
