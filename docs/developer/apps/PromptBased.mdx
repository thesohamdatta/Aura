...
description: "Customize Aura's behavior with prompts - no server required. Create specialized AI personalities and memory processors."
---

## What Are Prompt-Based Apps?

Prompt-based apps let you customize how Aura thinks and responds without writing any backend code. Simply define your prompt and Aura's behavior changes accordingly.

<CardGroup cols={2}>
  <Card title="No Server Required" icon="cloud-slash">
    Everything runs on Aura's infrastructure - just write your prompt
  </Card>
...
Chat prompts alter Aura's conversational style and knowledge base. Create specialized versions of Aura for different contexts or expertise areas.

```mermaid
flowchart LR
    U[User Message] --> O[Aura + Your Prompt]
...
<AccordionGroup>
  <Accordion title="How Chat Prompts Work" icon="gear">
    When a user chats with Aura while your app is enabled:

    1. Your prompt is injected into the system context
    2. Aura adopts the personality/expertise you defined
  ...
  <Accordion title="Example Use Cases" icon="lightbulb">
    - **Expert Personas**: Make Aura communicate like a historian, scientist, or creative writer
...
<Tip>
  A great chat prompt defines personality, expertise, and response style
  clearly.
</Tip>
...
<Tip>Be specific about what to extract and how to format the output.</Tip>
...
<Info>
  You can create apps with both chat AND memory prompts for comprehensive
  functionality.
</Info>
...
<Steps>
  <Step title="Define Your Purpose" icon="target">
    Decide what your app will do: - **Chat only**: Personality/expertise
    customization - **Memory only**: Information extraction - **Both**: Complete
    conversational experience with data extraction
  </Step>
  <Step title="Write Your Prompt(s)" icon="pen">
    Craft clear, specific prompts: - Define personality traits and expertise
    areas - Specify extraction criteria and output format - Include examples if
    helpful
  </Step>
...
<Steps>
  <Step title="Enable Developer Mode" icon="code">
    Open the Aura app → Settings → Enable Developer Mode
  </Step>
...
<CardGroup cols={2}>
  <Card title="Be Specific" icon="bullseye">
    Vague prompts lead to inconsistent results. Define exact behaviors, formats,
    and constraints.
  </Card>
  <Card title="Test Edge Cases" icon="flask">
    Try unusual inputs to ensure your prompt handles unexpected scenarios
    gracefully.
  </Card>
  ...
  <Card title="Iterate Quickly" icon="arrows-rotate">
    Start simple, test, and refine. Don't try to perfect the prompt on the first
    attempt.
  </Card>
</CardGroup>

<Warning>
  Avoid prompts that could generate harmful, misleading, or inappropriate
  content. Apps violating community guidelines will be removed.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="puzzle-piece"
    href="/doc/developer/apps/Introduction"
  >
