---
title: "Chat Tools"
description: "Add custom chat tools to your Aura app that extend Aura's capabilities in user conversations."
---

## Overview

**Chat Tools** allow your app to provide custom functions that become available in Aura chat when users install your app. These tools enable the AI assistant to interact with your service directly through natural language conversations.
...
When a user installs your app, these tools automatically become available in their Aura chat, allowing them to say things like "Send a message to #general saying hello" or "What Slack channels do I have?"
...
<Steps>
  <Step title="User Chats with Aura">
    User makes a request in natural language
  </Step>
  <Step title="AI Decides to Use Tool">
    Aura's AI determines your tool is the best way to fulfill the request
  </Step>
  <Step title="Aura Calls Your Endpoint">
    POST request with `uid`, `app_id`, `tool_name`, plus extracted parameters
  </Step>
  <Step title="Your Server Processes">
    Execute the action and return a result
  </Step>
  <Step title="User Sees Result">Aura displays your response to the user</Step>
</Steps>
...
Your backend server needs to expose endpoints that Aura will call when tools are invoked.
...
## Part 2: Adding Chat Tools in Aura App Store

Chat tools are defined via a **manifest endpoint** hosted on your server. When you create or update your app in the Aura App Store, Aura will automatically fetch the tool definitions from your manifest URL.
...
Create a JSON file at `/.well-known/aura-tools.json` on your server:
...
- `https://your-app.com/.well-known/aura-tools.json` (recommended)
- `https://your-app.com/aura/manifest.json`
- `https://your-app.com/api/tools-manifest`

### Step 3: Add Manifest URL in Aura App Store

<Steps>
  <Step title="Open Aura App">
    Navigate to Apps → Create App (or edit an existing app)
  </Step>
  <Step title="Select Capability">Choose External Integration capability</Step>
  <Step title="Enter Manifest URL">
    In the **Chat Tools Manifest URL** field, enter: ```
    https://your-app.com/.well-known/aura-tools.json ```
  </Step>
  <Step title="Submit">
    Aura will automatically fetch your tool definitions
  </Step>
</Steps>

<Note>
  **Endpoint URLs in Manifest**: You can use relative paths (e.g.,
  `/api/send_message`) in your manifest. Aura will automatically resolve them
...
@app.get("/.well-known/aura-tools.json")
async def get_aura_tools_manifest():
    return {
        "tools": [
            {
                "name": "search_songs",
...
<AccordionGroup>
  ...
  <Accordion title="Status Messages" icon="spinner">
    ...
    If not provided, Aura generates a default based on the tool name.
...
<Steps>
  ...
  <Step title="Create the App">
    Add your tool definitions in the Aura App Store
  </Step>
...
<AccordionGroup>
  <Accordion title="Tool Not Appearing in Chat" icon="eye-slash">
    - Verify the app is installed and enabled (`enabled: true`) - Check that
    `chat_tools` array is properly formatted - Ensure endpoints are accessible
    and return proper responses - Verify the app has `external_integration`
    capability
  </Accordion>
  <Accordion title="Tool Calls Failing" icon="circle-xmark">
    - Check endpoint logs for errors - Verify authentication is working - Ensure
    response format matches specification - Test endpoints directly with
    curl/Postman - Check that required parameters are being sent
  </Accordion>
  <Accordion title="AI Not Using Your Tool" icon="robot">
    - Improve tool description to be more specific about when to use it - Add
    examples in the description - Ensure tool name is descriptive - Make sure
    the description clearly states when the tool should be used
  </Accordion>
</AccordionGroup>
...
<Card
  ...
  icon="slack"
  href="/doc/developer/apps/examples/Slack"
>
  See a complete working example that demonstrates OAuth authentication,
  multiple chat tools, handling invocations, and error handling
</Card>
...
<CardGroup cols={2}>
  <Card title="Developer API" icon="code" href="/doc/developer/api">
    Access your own personal Aura data programmatically
  </Card>
  <Card
  ...
  <Card
    title="Notifications"
    icon="bell"
    href="/doc/developer/apps/Notifications"
  >
