...
description: "Programmatically create and read conversations and memories in users' Aura accounts using the Integration Import APIs."
---

Aura's Data Import APIs allow your app to programmatically interact with user data in the Aura ecosystem. Unlike triggers (which respond to events), these APIs let your app proactively create or read data.
...
<Steps>
  <Step title="Understand Aura App Basics">
    Familiarize yourself with [Aura app development](/doc/developer/apps/Introduction) and [integration apps](/doc/developer/apps/Integrations)
  </Step>
  <Step title="Create Your App">
    In the Aura mobile app:
  ...
  <Step title="User Enables Your App">
    The user must explicitly enable your app in their Aura account before you can access their data
  </Step>
</Steps>
...
Create new conversations in a user's Aura account from external sources like phone calls, emails, or messages.

### Endpoint

```
POST https://api.aura.me/v2/integrations/{app_id}/user/conversations?uid={user_id}
```
...
<AccordionGroup>
  <Accordion title="Example: Phone Call Transcript" icon="phone">
    ...
    response = requests.post(
        f"https://api.aura.me/v2/integrations/{APP_ID}/user/conversations?uid={USER_ID}",
...
Add facts and knowledge to a user's memory bank. You can either provide raw text (Aura will extract memories automatically) or provide explicit memory objects.

### Endpoint

```
POST https://api.aura.me/v2/integrations/{app_id}/user/memories?uid={user_id}
```
...
<Tabs>
  <Tab title="Extract from Text">
    Provide raw text and let Aura automatically extract relevant memories:
...
<AccordionGroup>
  <Accordion title="Example: Extract from Email" icon="envelope">
    ```python
    response = requests.post(
        f"https://api.aura.me/v2/integrations/{APP_ID}/user/memories?uid={USER_ID}",
        headers={"Authorization": f"Bearer {API_KEY}"},
        json={
...
Retrieve conversations from a user's Aura account.

### Endpoint

```
GET https://api.aura.me/v2/integrations/{app_id}/conversations?uid={user_id}
```
...
response = requests.get(
    f"https://api.aura.me/v2/integrations/{APP_ID}/conversations?uid={USER_ID}",
    headers={"Authorization": f"Bearer {API_KEY}"},
...
GET https://api.aura.me/v2/integrations/{app_id}/memories?uid={user_id}
...
```python
response = requests.get(
    f"https://api.aura.me/v2/integrations/{APP_ID}/memories?uid={USER_ID}",
    headers={"Authorization": f"Bearer {API_KEY}"},
    params={"limit": 100}
)
...
<AccordionGroup>
  <Accordion title="Journal Creator" icon="book">
    Automatically create conversation entries from a journaling app, syncing daily reflections to Aura
  </Accordion>
...
<Steps>
  ...
  <Step title="Verify in the Aura App">
    After creating content, open the Aura app to verify it appears correctly
  </Step>
...
The Aura team is expanding the available capabilities. Coming soon:
...
<CardGroup cols={2}>
  ...
  <Card title="Submit Your App" icon="paper-plane" href="/doc/developer/apps/Submitting">
    Guidelines for submitting your app to the Aura store
  </Card>
  <Card title="Chat Tools" icon="wrench" href="/doc/developer/apps/ChatTools">
    Add custom tools to Aura's AI chat
  </Card>
</CardGroup>
```
