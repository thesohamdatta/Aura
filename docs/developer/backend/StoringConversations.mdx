...
description: "This document outlines how Aura stores conversation data and extracts memories from conversations."
---

## Overview

Aura uses a **dual-collection architecture** for storing user data:

<CardGroup cols={2}>
  <Card title="Conversations" icon="comments">
    **Primary storage** for recorded interactions - transcripts, audio,
...
flowchart TD
    ...
    PC --> EM[_extract_memories<br/>facts → memories]
...
<Steps>
  ...
  <Step title="Processing" icon="gear">
    `process_conversation()` in `utils/conversations/process_conversation.py`
    handles the logic
  </Step>
...
| `source`                | enum     | Source device (aura, phone, desktop, openglass, etc.)                 |
...
<Note>
  Legacy categories (`core`, `hobbies`, `lifestyle`, `interests`, `habits`,
  `work`, `skills`, `learnings`, `other`) are automatically mapped to the new
  primary categories for backward compatibility.
</Note>
...
<Warning>
  Vectors are created ONCE during initial processing. Reprocessed conversations
  do NOT update their vectors.
</Warning>
