...
<Card
  title="GET /v1/dev/user/conversations"
  icon="download"
  color="#22c55e"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Query Parameters" icon="sliders" defaultOpen={true}>
    ...
    |-----------|------|---------|-------------| | `limit` | integer | 25 |
    Maximum number to return | | `offset` | integer | 0 | Number to skip | |
...
<Tabs>
  <Tab title="cURL">
    ...
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.aura.me/v1/dev/user/conversations?limit=10&include_transcript=true"
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    response = requests.get(
        "https://api.aura.me/v1/dev/user/conversations",
...
<Card
  title="POST /v1/dev/user/conversations"
  icon="upload"
  color="#3b82f6"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Request Body" icon="code" defaultOpen={true}>
    ...
    |-----------|------|----------|-------------| | `text` | string | **Yes** |
    The conversation text/transcript (1-100,000 characters) | | `text_source` |
    string | No | Source type: `audio_transcript`, `message`, or `other_text`
    (default: `other_text`) | | `text_source_spec` | string | No | Additional
    source info (e.g., 'email', 'slack', 'whatsapp') | | `started_at` | datetime
    | No | When conversation started (defaults to now) | | `finished_at` |
    datetime | No | When conversation finished (defaults to `started_at` + 5
    minutes) | | `language` | string | No | Language code (default: 'en') | |
    `geolocation` | object | No | Geolocation data with `latitude` and
    `longitude` |
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/user/conversations" \
...
<Note>
  Memories and action items are extracted asynchronously. Use the GET endpoints
  to retrieve them after creation.
</Note>
...
<Card
  title="POST /v1/dev/user/conversations/from-segments"
  icon="waveform-lines"
  color="#3b82f6"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Request Body" icon="code" defaultOpen={true}>
    | Parameter | Type | Required | Description |
    |-----------|------|----------|-------------| | `transcript_segments` |
    array | **Yes** | List of transcript segments (1-500 segments) | | `source`
    | string | No | Source of conversation (default: `external_integration`) | |
    `started_at` | datetime | No | When conversation started (defaults to now) |
    | `finished_at` | datetime | No | When conversation finished (calculated
    from last segment if not provided) | | `language` | string | No | Language
    code (default: 'en') | | `geolocation` | object | No | Geolocation data |
  </Accordion>
  <Accordion title="Transcript Segment Fields" icon="list">
    | Field | Type | Required | Description |
    |-------|------|----------|-------------| | `text` | string | **Yes** | The
    spoken text | | `speaker` | string | No | Speaker identifier (e.g.,
    'SPEAKER_00', 'SPEAKER_01'). Default: 'SPEAKER_00' | | `speaker_id` |
    integer | No | Numeric speaker ID (auto-calculated if not provided) | |
    `is_user` | boolean | No | Whether this segment is from the user. Default:
    `false` | | `person_id` | string | No | ID of known person speaking | |
  ...
  <Accordion title="Available Sources" icon="microchip">
    `aura`, `friend`, `openglass`, `phone`, `desktop`, `apple_watch`, `bee`,
    `plaud`, `frame`, `screenpipe`, `workflow`, `sdcard`, `external_integration`
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/user/conversations/from-segments" \
      -H "Authorization: Bearer $API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "transcript_segments": [
  ...
  <Tab title="Python">
    ...
    response = requests.post(
        "https://api.aura.me/v1/dev/user/conversations/from-segments",
        headers={
            "Authorization": f"Bearer {API_KEY}",
            "Content-Type": "application/json"
        },
...
<Note>
  ...
  `transcript_segments` field and can be retrieved with
...
<Card
  title="GET /v1/dev/user/conversations/{conversation_id}"
  icon="file"
  color="#22c55e"
  horizontal
>
...
<AccordionGroup>
  ...
  <Accordion title="Query Parameters" icon="sliders" defaultOpen={true}>
    | Parameter | Type | Default | Description |
    |-----------|------|---------|-------------| | `include_transcript` |
    boolean | false | Include full transcript segments |
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.aura.me/v1/dev/user/conversations/conv_456?include_transcript=true"
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    response = requests.get(
        "https://api.aura.me/v1/dev/user/conversations/conv_456",
        headers={"Authorization": f"Bearer {API_KEY}"},
...
<Card
  title="PATCH /v1/dev/user/conversations/{conversation_id}"
  icon="pen"
  color="#a855f7"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X PATCH "https://api.aura.me/v1/dev/user/conversations/conv_456" \
      -H "Authorization: Bearer $API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
...
<Tip>
  Use `discarded: true` to hide a conversation from the main list without
...
<Card
  title="DELETE /v1/dev/user/conversations/{conversation_id}"
  icon="trash"
  color="#ef4444"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X DELETE "https://api.aura.me/v1/dev/user/conversations/conv_456" \
      -H "Authorization: Bearer $API_KEY"
    ```
  </Tab>
  <Tab title="Python">
...
<Warning>
  This action is permanent. Deleted conversations and their associated photos
  cannot be recovered. Consider using `PATCH` with `discarded: true` to hide a
...
<Tabs>
  <Tab title="Bash">
    ...
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.aura.me/v1/dev/user/conversations?limit=1000&include_transcript=true" \
  ...
  <Tab title="JavaScript">
    ...
    async function exportConversations() {
      ...
      while (true) {
        const response = await fetch(
          `https://api.aura.me/v1/dev/user/conversations?limit=${limit}&offset=${offset}&include_transcript=true`,
          { headers: { Authorization: `Bearer ${API_KEY}` } }
        );
