...
<Card
  title="GET /v1/dev/user/action-items"
  icon="download"
  color="#22c55e"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Query Parameters" icon="sliders" defaultOpen={true}>
    ...
    |-----------|------|---------|-------------| | `limit` | integer | 100 |
    Maximum number of items to return | | `offset` | integer | 0 | Number of
    items to skip | | `completed` | boolean | - | Filter by completion status |
...
<Tabs>
  <Tab title="cURL">
    ...
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.aura.me/v1/dev/user/action-items?completed=false&limit=50"
    ```
  </Tab>
  <Tab title="Python">
    ```python
    ...
    response = requests.get(
        "https://api.aura.me/v1/dev/user/action-items",
...
<Card
  title="POST /v1/dev/user/action-items"
  icon="upload"
  color="#3b82f6"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Request Body" icon="code" defaultOpen={true}>
    ...
    |-----------|------|----------|-------------| | `description` | string |
    **Yes** | The action item description (1-500 characters) | | `completed` |
    boolean | No | Whether completed (default: `false`) | | `due_at` | datetime
    | No | Due date in ISO 8601 format with timezone |
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/user/action-items" \
...
<Tip>
  Push notifications are automatically sent to the Aura app if a `due_at` date
  is provided.
</Tip>
...
<Card
  title="POST /v1/dev/user/action-items/batch"
  icon="layer-group"
  color="#3b82f6"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/user/action-items/batch" \
      -H "Authorization: Bearer $API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "action_items": [
  ...
  <Tab title="Python">
    ...
    response = requests.post(
        "https://api.aura.me/v1/dev/user/action-items/batch",
        headers={
            "Authorization": f"Bearer {API_KEY}",
            "Content-Type": "application/json"
        },
...
<Card
  title="PATCH /v1/dev/user/action-items/{action_item_id}"
  icon="pen"
  color="#a855f7"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X PATCH "https://api.aura.me/v1/dev/user/action-items/action_abc123" \
      -H "Authorization: Bearer $API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "completed": true
  ...
  <Tab title="Python">
    ...
    response = requests.patch(
        "https://api.aura.me/v1/dev/user/action-items/action_abc123",
        headers={
            "Authorization": f"Bearer {API_KEY}",
            "Content-Type": "application/json"
        },
...
<Tip>
  When updating `due_at`, a push notification is sent to the Aura app to remind
  the user.
</Tip>
...
<Card
  title="DELETE /v1/dev/user/action-items/{action_item_id}"
  icon="trash"
  color="#ef4444"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X DELETE "https://api.aura.me/v1/dev/user/action-items/action_abc123" \
      -H "Authorization: Bearer $API_KEY"
    ```
  </Tab>
  <Tab title="Python">
...
Import tasks from an external task management system into Aura:
...
<Tabs>
  <Tab title="Python">
    ...
    # Convert to Aura action items format
    action_items = [
        {
...
<Tabs>
  <Tab title="cURL">
    ...
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.aura.me/v1/dev/user/action-items?completed=false"
    ```
  </Tab>
  <Tab title="Python">
    ```python
  ...
  <Tab title="JavaScript">
    ...
    const response = await fetch(
      "https://api.aura.me/v1/dev/user/action-items?completed=false&limit=100",
      { headers: { Authorization: `Bearer ${API_KEY}` } }
    );
