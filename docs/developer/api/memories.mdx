...
<Card
  title="GET /v1/dev/user/memories"
  icon="download"
  color="#22c55e"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Query Parameters" icon="sliders" defaultOpen={true}>
    ...
    |-----------|------|---------|-------------| | `limit` | integer | 25 |
    Maximum number of memories to return | | `offset` | integer | 0 | Number of
    memories to skip | | `categories` | string | - | Comma-separated list (e.g.,
    `"interesting,system"`) |
  </Accordion>
</AccordionGroup>
...
<Tabs>
  <Tab title="cURL">
    ...
    curl -H "Authorization: Bearer $API_KEY" \
      "https://api.aura.me/v1/dev/user/memories?limit=50&categories=interesting"
    ```
  </Tab>
  <Tab title="Python">
    ```python
...
<Card
  title="POST /v1/dev/user/memories"
  icon="upload"
  color="#3b82f6"
  horizontal
>
...
<AccordionGroup>
  <Accordion title="Request Body" icon="code" defaultOpen={true}>
    ...
    |-----------|------|----------|-------------| | `content` | string | **Yes**
    | The memory content (1-500 characters) | | `category` | string | No |
    `interesting`, `system`, or `manual` (auto-categorizes if not provided) | |
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/user/memories" \
...
<Card
  title="POST /v1/dev/user/memories/batch"
  icon="layer-group"
  color="#3b82f6"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/user/memories/batch" \
      -H "Authorization: Bearer $API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
        "memories": [
  ...
  <Tab title="Python">
    ...
    response = requests.post(
        "https://api.aura.me/v1/dev/user/memories/batch",
        headers={
            "Authorization": f"Bearer {API_KEY}",
            "Content-Type": "application/json"
        },
...
<Card
  title="PATCH /v1/dev/user/memories/{memory_id}"
  icon="pen"
  color="#a855f7"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X PATCH "https://api.aura.me/v1/dev/user/memories/mem_xyz789" \
      -H "Authorization: Bearer $API_KEY" \
      -H "Content-Type: application/json" \
      -d '{
  ...
  <Tab title="Python">
    ...
    response = requests.patch(
        "https://api.aura.me/v1/dev/user/memories/mem_xyz789",
        headers={
            "Authorization": f"Bearer {API_KEY}",
            "Content-Type": "application/json"
        },
...
<Card
  title="DELETE /v1/dev/user/memories/{memory_id}"
  icon="trash"
  color="#ef4444"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X DELETE "https://api.aura.me/v1/dev/user/memories/mem_xyz789" \
      -H "Authorization: Bearer $API_KEY"
    ```
  </Tab>
  <Tab title="Python">
...
<CardGroup cols={2}>
  <Card title="system" icon="gear" color="#3b82f6">
    Useful context for the AI: preferences, work details, relationships,
    logistical info. Example: "User prefers dark mode"
  </Card>
  <Card title="interesting" icon="star" color="#f59e0b">
    Shareable, notable facts — things the user would excitedly tell someone at
    dinner. Example: "User climbed Mount Kilimanjaro"
  </Card>
</CardGroup>
