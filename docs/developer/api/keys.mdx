...
<Note>
  This endpoint requires authentication with an existing API key or user
  session. The secret key values are not returned (only the prefix is shown).
</Note>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST "https://api.aura.me/v1/dev/keys" \
...
<Accordion title="Response Example" icon="code" defaultOpen={true}>
  ```json
  {
    ...
    "key_prefix": "aura_dev_ghi789",
    "key": "aura_dev_ghi789_full_secret_key_here",
...
<Warning>
  The full API key (`key` field) is only returned once during creation. Store it
  securely immediately - you won't be able to see it again!
</Warning>
...
<Card
  title="DELETE /v1/dev/keys/{key_id}"
  icon="trash"
  color="#ef4444"
  horizontal
>
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X DELETE "https://api.aura.me/v1/dev/keys/key_789ghi" \
      -H "Authorization: Bearer $API_KEY"
    ```
  </Tab>
  <Tab title="Python">
    ...
    response = requests.delete(
        f"https://api.aura.me/v1/dev/keys/key_789ghi",
        headers={"Authorization": f"Bearer {API_KEY}"}
    )
  ...
  <Tab title="JavaScript">
    ```javascript
    const response = await fetch(
      "https://api.aura.me/v1/dev/keys/key_789ghi",
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${API_KEY}` }
      }
...
<Tip>If a key is compromised, revoke it immediately and create a new one.</Tip>
...
<CardGroup cols={2}>
  <Card title="Use Descriptive Names" icon="tag" color="#22c55e">
    Name keys after their purpose (e.g., "Analytics Dashboard", "Zapier
    Integration")
  </Card>
...
<Tabs>
  <Tab title="Python">
    ...
    def revoke_key(key_id):
        """Revoke an API key"""
        response = requests.delete(
            f"https://api.aura.me/v1/dev/keys/{key_id}",
            headers=headers
        )
  ...
  <Tab title="JavaScript">
    ...
    async function listKeys() {
      const response = await fetch("https://api.aura.me/v1/dev/keys", { headers });
      return response.json();
    }

    async function createKey(name) {
      const response = await fetch("https://api.aura.me/v1/dev/keys", {
        method: "POST",
        headers,
    ...
    async function revokeKey(keyId) {
      const response = await fetch(`https://api.aura.me/v1/dev/keys/${keyId}`, {
        method: "DELETE",
        headers
      });
...
You can also manage API keys directly in the Aura app:

<Steps>
  <Step title="Open Aura App" icon="mobile">
    Launch the Aura app on your device
  </Step>
  <Step title="Navigate to Settings" icon="gear">
    Go to **Settings → Developer**
  </Step>
  <Step title="Manage Keys" icon="key">
    Under "Developer API Keys" you can: - View all your keys - Create new keys -
    Delete existing keys
  </Step>
</Steps>

<Info>
  Keys created in the app and via the API are the same - you can manage them
  from either place.
</Info>
