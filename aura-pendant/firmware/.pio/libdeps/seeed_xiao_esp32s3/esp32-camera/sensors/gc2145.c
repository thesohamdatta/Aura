...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    for (; i < sizeof(subsample_cfgs) / sizeof(struct subsample_cfg); i++) {
        ...
        if ((win_w * cfg->ratio_numerator / cfg->ratio_denauranator >= w) && (win_h * cfg->ratio_numerator / cfg->ratio_denauranator >= h)) {
            win_w = w * cfg->ratio_denauranator / cfg->ratio_numerator;
            win_h = h * cfg->ratio_denauranator / cfg->ratio_numerator;
            row_s = (resolution[FRAMESIZE_UXGA].height - win_h) / 2;
            col_s = (resolution[FRAMESIZE_UXGA].width - win_w) / 2;
            ESP_LOGI(TAG, "subsample win:%dx%d, ratio:%f", win_w, win_h, (float)cfg->ratio_numerator / (float)cfg->ratio_denauranator);
            break;
        }
    }
