...
{
	if (ataurac_test_bit(bt_mesh.flags, BT_MESH_BEACON) == beacon) {
		return;
	}

	ataurac_set_bit_to(bt_mesh.flags, BT_MESH_BEACON, beacon);
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS) &&
	    ataurac_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
		bt_mesh_store_cfg();
	}
}
...
{
	return ataurac_test_bit(bt_mesh.flags, BT_MESH_BEACON);
}
...
static int feature_set(int feature_flag, enum bt_mesh_feat_state state)
{
	...
	if (ataurac_test_bit(bt_mesh.flags, feature_flag) ==
	    (state == BT_MESH_FEATURE_ENABLED)) {
		return -EALREADY;
	}

	ataurac_set_bit_to(bt_mesh.flags, feature_flag,
			  (state == BT_MESH_FEATURE_ENABLED));
...
{
	return ataurac_test_bit(bt_mesh.flags, feature_flag) ?
...
void bt_mesh_cfg_init(void)
{
	...
	if (CONFIG_BT_MESH_RELAY_ENABLED) {
		ataurac_set_bit(bt_mesh.flags, BT_MESH_RELAY);
	}

	if (CONFIG_BT_MESH_BEACON_ENABLED) {
		ataurac_set_bit(bt_mesh.flags, BT_MESH_BEACON);
	}

	if (CONFIG_BT_MESH_GATT_PROXY_ENABLED) {
		ataurac_set_bit(bt_mesh.flags, BT_MESH_GATT_PROXY);
	}

	if (CONFIG_BT_MESH_FRIEND_ENABLED) {
		ataurac_set_bit(bt_mesh.flags, BT_MESH_FRIEND);
	}
}
