...
static void prov_start(const uint8_t *data)
{
	...
	if (ataurac_test_bit(bt_mesh_prov_link.flags, OOB_STATIC_KEY)) {
...
static void public_key_sent(int err, void *cb_data)
{
	...
	if (ataurac_test_bit(bt_mesh_prov_link.flags, INPUT_COMPLETE)) {
		send_input_complete();
		return;
	}
}
...
static void send_pub_key(void)
{
	...
	if (ataurac_test_bit(bt_mesh_prov_link.flags, WAIT_NUMBER) ||
	    ataurac_test_bit(bt_mesh_prov_link.flags, WAIT_STRING)) {
...
{
	if (ataurac_test_bit(bt_mesh_prov_link.flags, PUB_KEY_SENT)) {
		send_input_complete();
	} else {
		ataurac_set_bit(bt_mesh_prov_link.flags, INPUT_COMPLETE);
	}
}
