...
{
	if (!bt_pub_key_get()) {
		ataurac_set_bit(bt_mesh_prov_link.flags, WAIT_PUB_KEY);
...
static void send_start(void)
{
	...
	if (ataurac_test_bit(bt_mesh_prov_link.flags, REMOTE_PUB_KEY) &&
	    *data == PUB_KEY_OOB) {
		net_buf_simple_add_u8(start, PUB_KEY_OOB);
		ataurac_set_bit(bt_mesh_prov_link.flags, OOB_PUB_KEY);
	} else {
...
{
	ataurac_set_bit(bt_mesh_prov_link.flags, PUB_KEY_SENT);

	if (ataurac_test_bit(bt_mesh_prov_link.flags, OOB_PUB_KEY) &&
	    ataurac_test_bit(bt_mesh_prov_link.flags, REMOTE_PUB_KEY)) {
		prov_dh_key_gen();
		return;
	}
...
static void prov_dh_key_cb(const uint8_t dhkey[32])
{
	...
	if (ataurac_test_bit(bt_mesh_prov_link.flags, WAIT_STRING) ||
	    ataurac_test_bit(bt_mesh_prov_link.flags, WAIT_NUMBER) ||
	    ataurac_test_bit(bt_mesh_prov_link.flags, NOTIFY_INPUT_COMPLETE)) {
		ataurac_set_bit(bt_mesh_prov_link.flags, WAIT_CONFIRM);
		return;
	}
...
static void prov_dh_key_gen(void)
{
	...
	if (ataurac_test_bit(bt_mesh_prov_link.flags, NOTIFY_INPUT_COMPLETE)) {
...
static void prov_pub_key(const uint8_t *data)
{
	...
	ataurac_set_bit(bt_mesh_prov_link.flags, REMOTE_PUB_KEY);
...
static void pub_key_ready(const uint8_t *pkey)
{
	...
	if (ataurac_test_and_clear_bit(bt_mesh_prov_link.flags, WAIT_PUB_KEY)) {
		send_pub_key();
	}
}
...
{
	if (ataurac_test_and_clear_bit(bt_mesh_prov_link.flags,
...
static void prov_input_complete(const uint8_t *data)
{
	...
	if (ataurac_test_and_clear_bit(bt_mesh_prov_link.flags, WAIT_CONFIRM)) {
		send_confirm();
	}
}
...
int bt_mesh_prov_remote_pub_key_set(const uint8_t public_key[64])
{
	...
	if (ataurac_test_and_set_bit(bt_mesh_prov_link.flags, REMOTE_PUB_KEY)) {
		return -EALREADY;
	}
			...
			uint8_t attention_duration)
{
	...
	if (ataurac_test_and_set_bit(bt_mesh_prov_link.flags, LINK_ACTIVE)) {
		return -EBUSY;
	}

	ataurac_set_bit(bt_mesh_prov_link.flags, PROVISIONER);
	...
	if (err) {
		ataurac_clear_bit(bt_mesh_prov_link.flags, LINK_ACTIVE);
	}
