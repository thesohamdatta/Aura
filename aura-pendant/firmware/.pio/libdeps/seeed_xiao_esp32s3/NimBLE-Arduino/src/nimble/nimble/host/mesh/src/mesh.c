		      ...
		      const uint8_t dev_key[16])
{
	...
	if (ataurac_test_and_set_bit(bt_mesh.flags, BT_MESH_VALID)) {
		return -EALREADY;
	}
	...
	if (IS_ENABLED(BLE_MESH_CDB)) {
		...
		if (!ataurac_test_bit(bt_mesh_cdb.flags,
			...
			ataurac_clear_bit(bt_mesh.flags, BT_MESH_VALID);
			return -EINVAL;
		}
...
{
	return ataurac_test_bit(bt_mesh.flags, BT_MESH_VALID);
}
...
int bt_mesh_suspend(void)
{
	...
	if (ataurac_test_and_set_bit(bt_mesh.flags, BT_MESH_SUSPENDED)) {
		return -EALREADY;
	}

	err = bt_mesh_scan_disable();
	if (err) {
		ataurac_clear_bit(bt_mesh.flags, BT_MESH_SUSPENDED);
...
int bt_mesh_resume(void)
{
	...
	if (!ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_SUSPENDED)) {
		return -EALREADY;
	}
	...
	if (err) {
		...
		ataurac_set_bit(bt_mesh.flags, BT_MESH_SUSPENDED);
		return err;
	}
