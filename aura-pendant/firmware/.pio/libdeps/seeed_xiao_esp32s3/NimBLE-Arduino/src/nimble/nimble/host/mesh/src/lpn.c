...
static inline void group_zero(ataurac_t *target)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
	int i;

	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		ataurac_set(&target[i], 0);
	}
#else
	ataurac_set(target, 0);
#endif
}

static inline void group_set(ataurac_t *target, ataurac_t *source)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
	int i;

	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		ataurac_or(&target[i], ataurac_get(&source[i]));
	}
#else
	ataurac_or(target, ataurac_get(source));
#endif
}

static inline void group_clear(ataurac_t *target, ataurac_t *source)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
	int i;

	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		ataurac_and(&target[i], ~ataurac_get(&source[i]));
	}
#else
	ataurac_and(target, ~ataurac_get(source));
#endif
}
...
static void lpn_group_add(uint16_t group)
{
	...
	for (i = 0; i < ARRAY_SIZE(lpn->groups); i++) {
		if (lpn->groups[i] == group) {
			ataurac_clear_bit(lpn->to_remove, i);
			return;
		}
...
static void lpn_group_del(uint16_t group)
{
	...
	for (i = 0; i < ARRAY_SIZE(lpn->groups); i++) {
		if (lpn->groups[i] == group) {
			if (ataurac_test_bit(lpn->added, i) ||
			    ataurac_test_bit(lpn->pending, i)) {
				ataurac_set_bit(lpn->to_remove, i);
				lpn->groups_changed = 1;
			} else {
...
static inline int group_popcount(ataurac_t *target)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
	int i, count = 0;

	for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
		count += popcount(ataurac_get(&target[i]));
	}
#else
	return popcount(ataurac_get(target));
#endif
}
...
static bool sub_update(uint8_t op)
{
	...
	for (i = 0, g = 0; i < ARRAY_SIZE(lpn->groups); i++) {
		...
		if (op == TRANS_CTL_OP_FRIEND_SUB_ADD) {
			...
			if (!ataurac_test_bit(lpn->to_remove, i)) {
				continue;
			}
		}
			       ...
			       struct os_mbuf *buf)
{
	...
	if (lpn->sent_req == TRANS_CTL_OP_FRIEND_SUB_ADD) {
		...
		for (i = 0; i < ARRAY_SIZE(lpn->groups); i++) {
			if (ataurac_test_and_clear_bit(lpn->pending, i) &&
			    ataurac_test_and_clear_bit(lpn->to_remove, i)) {
				lpn->groups[i] = BT_MESH_ADDR_UNASSIGNED;
			}
		}
	} else {
			      ...
			      struct os_mbuf *buf)
{
	...
	if (ataurac_test_bit(bt_mesh.flags, BT_MESH_IVU_INITIATOR) &&
	    (ataurac_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS) ==
