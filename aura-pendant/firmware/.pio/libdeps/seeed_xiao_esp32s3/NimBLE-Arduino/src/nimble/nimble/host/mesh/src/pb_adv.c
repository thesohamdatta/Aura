...
struct pb_adv {
	...
	ATAURAC_DEFINE(flags, ADV_NUM_FLAGS);
...
static void reset_adv_link(void)
{
	...
	if (ataurac_test_bit(link.flags, ADV_PROVISIONER)) {
...
static void ack_complete(uint16_t duration, int err, void *user_data)
{
	...
	ataurac_clear_bit(link.flags, ADV_ACK_PENDING);
}

static bool ack_pending(void)
{
	return ataurac_test_bit(link.flags, ADV_ACK_PENDING);
}
...
static void prov_failed(uint8_t err)
{
	...
	ataurac_set_bit(link.flags, ADV_LINK_INVALID);
}
...
static void prov_msg_recv(void)
{
	...
	if (ataurac_test_bit(link.flags, ADV_LINK_INVALID)) {
...
static void gen_prov_ack_send(uint8_t xact_id)
{
	...
	struct os_mbuf *buf;
	bool pending = ataurac_test_and_set_bit(link.flags, ADV_ACK_PENDING);
...
static void gen_prov_ack(struct prov_rx *rx, struct os_mbuf *buf)
{
	...
	if (rx->xact_id == link.tx.id) {
		...
		if (!ataurac_test_bit(link.flags, ADV_LINK_CLOSING)) {
			prov_clear_tx();
		}
...
static void gen_prov_ctl(struct prov_rx *rx, struct os_mbuf *buf)
{
	...
	case LINK_OPEN:
		...
		break;
	case LINK_ACK:
		if (!ataurac_test_bit(link.flags, ADV_LINK_ACTIVE)) {
			return;
		}
...
static void link_open(struct prov_rx *rx, struct os_mbuf *buf)
{
	...
	link.id = rx->link_id;
	ataurac_set_bit(link.flags, ADV_LINK_ACTIVE);
	net_buf_simple_reset(link.rx.buf);
...
static void link_ack(struct prov_rx *rx, struct os_mbuf *buf)
{
	...
	if (ataurac_test_bit(link.flags, ADV_PROVISIONER)) {
		if (ataurac_test_and_set_bit(link.flags, ADV_LINK_ACK_RECVD)) {
			return;
		}
...
void bt_mesh_pb_adv_recv(struct os_mbuf *buf)
{
	...
	if (ataurac_test_bit(link.flags, ADV_LINK_ACTIVE) && link.id != rx.link_id) {
		return;
	}
			  ...
			  const struct prov_bearer_cb *cb, void *cb_data)
{
	...
	if (ataurac_test_and_set_bit(link.flags, ADV_LINK_ACTIVE)) {
		return -EBUSY;
	}

	ataurac_set_bit(link.flags, ADV_PROVISIONER);
...
{
	if (ataurac_test_and_set_bit(link.flags, ADV_LINK_CLOSING)) {
		return;
	}
