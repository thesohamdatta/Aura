...
struct bt_mesh_lpn {
	...
	ATAURAC_DEFINE(added, LPN_GROUPS);
	ATAURAC_DEFINE(pending, LPN_GROUPS);
	ATAURAC_DEFINE(to_remove, LPN_GROUPS);
};
...
struct bt_mesh_net {
	...
	ATAURAC_DEFINE(flags, BT_MESH_FLAG_COUNT);
...
#define BT_MESH_NET_IVI_TX (bt_mesh.iv_index - \
			    ataurac_test_bit(bt_mesh.flags, \
