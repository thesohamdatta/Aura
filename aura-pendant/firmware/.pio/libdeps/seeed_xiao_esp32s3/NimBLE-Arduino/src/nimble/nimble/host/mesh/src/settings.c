...
static int iv_set(int argc, char **argv, char *val)
{
	...
	if (!val) {
		bt_mesh.iv_index = 0U;
		ataurac_clear_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS);
		return 0;
	}
	...
	ataurac_set_bit_to(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS, iv.iv_update);
...
static int cdb_net_set(int argc, char *val)
{
	...
	if (net.iv_update) {
		ataurac_set_bit(bt_mesh_cdb.flags, BT_MESH_CDB_IVU_IN_PROGRESS);
	}

	ataurac_set_bit(bt_mesh_cdb.flags, BT_MESH_CDB_VALID);
...
static int cdb_node_set(int argc, char *str)
{
	...
	if (val.flags & F_NODE_CONFIGURED) {
		ataurac_set_bit(node->flags, BT_MESH_CDB_NODE_CONFIGURED);
	}
...
static int mesh_commit(void)
{
	...
	ataurac_set_bit(bt_mesh.flags, BT_MESH_VALID);
...
static void schedule_store(int flag)
{
	...
	ataurac_set_bit(bt_mesh.flags, flag);

	if (ataurac_get(bt_mesh.flags) & NO_WAIT_PENDING_BITS) {
		timeout = K_NO_WAIT;
	} else if (ataurac_test_bit(bt_mesh.flags, BT_MESH_RPL_PENDING) &&
		   (!(ataurac_get(bt_mesh.flags) & GENERIC_PENDING_BITS) ||
...
static void store_pending_iv(void)
{
	...
	iv.iv_update = ataurac_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS);
...
static void store_pending_cdb(void)
{
	...
	net.iv_update = ataurac_test_bit(bt_mesh_cdb.flags,
...
static void store_cdb_node(const struct bt_mesh_cdb_node *node)
{
	...
	if (ataurac_test_bit(node->flags, BT_MESH_CDB_NODE_CONFIGURED)) {
...
static void store_pending(struct ble_npl_event *work)
{
	...
	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_RPL_PENDING)) {
		if (ataurac_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
	...
	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_KEYS_PENDING)) {
		store_pending_keys();
	}

	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_NET_PENDING)) {
		if (ataurac_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
			store_pending_net();
		} else {
			clear_net();
	...
	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_IV_PENDING)) {
		if (ataurac_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
			store_pending_iv();
		} else {
			clear_iv();
	...
	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_SEQ_PENDING)) {
		store_pending_seq();
	}

	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_HB_PUB_PENDING)) {
		store_pending_hb_pub();
	}

	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_CFG_PENDING)) {
		if (ataurac_test_bit(bt_mesh.flags, BT_MESH_VALID)) {
			store_pending_cfg();
		} else {
			clear_cfg();
	...
	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_MOD_PENDING)) {
		bt_mesh_model_foreach(store_pending_mod, NULL);
	}

	if (ataurac_test_and_clear_bit(bt_mesh.flags, BT_MESH_VA_PENDING)) {
		store_pending_va();
	}

#if MYNEWT_VAL(BLE_MESH_CDB)
	if (IS_ENABLED(CONFIG_BT_MESH_CDB)) {
		if (ataurac_test_and_clear_bit(bt_mesh_cdb.flags,
					      BT_MESH_CDB_SUBNET_PENDING)) {
			if (ataurac_test_bit(bt_mesh_cdb.flags,
					    BT_MESH_CDB_VALID)) {
				store_pending_cdb();
			} else {
				clear_cdb();
...
{
	ataurac_set_bit(bt_mesh_cdb.flags, flag);
