...
int bt_mesh_cdb_create(const uint8_t key[16])
{
	...
	if (ataurac_test_and_set_bit(bt_mesh_cdb.flags,
				    BT_MESH_CDB_VALID)) {
		return -EALREADY;
	}
...
void bt_mesh_cdb_clear(void)
{
	...
	ataurac_clear_bit(bt_mesh_cdb.flags, BT_MESH_CDB_VALID);
...
void bt_mesh_cdb_iv_update(uint32_t iv_index, bool iv_update)
{
	...
	ataurac_set_bit_to(bt_mesh_cdb.flags, BT_MESH_CDB_IVU_IN_PROGRESS,
			  iv_update);
...
uint8_t bt_mesh_cdb_subnet_flags(const struct bt_mesh_cdb_subnet *sub)
{
	...
	if (ataurac_test_bit(bt_mesh_cdb.flags, BT_MESH_CDB_IVU_IN_PROGRESS)) {
