		       ...
		       uint32_t iv_index)
{
	...
	ataurac_set_bit_to(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS,
...
{
	ataurac_set_bit_to(bt_mesh.flags, BT_MESH_IVU_TEST, enable);
...
bool bt_mesh_iv_update(void)
{
	...
	if (ataurac_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS)) {
		bt_mesh_net_iv_update(bt_mesh.iv_index, false);
	} else {
		bt_mesh_net_iv_update(bt_mesh.iv_index + 1, true);
	}

	return ataurac_test_bit(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS);
}
...
bool bt_mesh_net_iv_update(uint32_t iv_index, bool iv_update)
{
	...
	if (!(IS_ENABLED(CONFIG_BT_MESH_IV_UPDATE_TEST) &&
	      ataurac_test_bit(bt_mesh.flags, BT_MESH_IVU_TEST))) {
	...
	if (!iv_update && bt_mesh_tx_in_progress()) {
		...
		ataurac_set_bit(bt_mesh.flags, BT_MESH_IVU_PENDING);
		return false;
	}

do_update:
	ataurac_set_bit_to(bt_mesh.flags, BT_MESH_IVU_IN_PROGRESS, iv_update);
