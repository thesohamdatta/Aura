...
int bt_mesh_prov_reset_state(void (*func)(const uint8_t key[64]))
{
	...
	ataurac_clear(bt_mesh_prov_link.flags);
...
int bt_mesh_prov_auth(uint8_t method, uint8_t action, uint8_t size)
{
	...
	case AUTH_METHOD_STATIC:
		...
		ataurac_set_bit(bt_mesh_prov_link.flags, OOB_STATIC_KEY);
	...
	case AUTH_METHOD_OUTPUT:
		...
		ataurac_set_bit(bt_mesh_prov_link.flags, NOTIFY_INPUT_COMPLETE);
	...
	case AUTH_METHOD_INPUT:
		...
		if (input == BT_MESH_ENTER_STRING) {
			ataurac_set_bit(bt_mesh_prov_link.flags, WAIT_STRING);
		} else {
			ataurac_set_bit(bt_mesh_prov_link.flags, WAIT_NUMBER);
		}
...
int bt_mesh_input_number(uint32_t num)
{
	...
	if (!ataurac_test_and_clear_bit(bt_mesh_prov_link.flags, WAIT_NUMBER)) {
		return -EINVAL;
	}
...
int bt_mesh_input_string(const char *str)
{
	...
	if (!ataurac_test_and_clear_bit(bt_mesh_prov_link.flags, WAIT_STRING)) {
		return -EINVAL;
	}
...
{
	return ataurac_test_bit(bt_mesh_prov_link.flags, LINK_ACTIVE);
}
...
{
	ataurac_set_bit(bt_mesh_prov_link.flags, LINK_ACTIVE);
